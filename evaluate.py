...
def process_single_file(row):
    # row = index, Series
    _, file = row
    features = audiofile_to_input_vector(
        file.wav_filename, N_FEATURES, N_CONTEXT)
...
def calculate_report(labels, decodings, distances, losses):
    ...
    '''
    samples = pmap(process_decode_result, zip(
...
def main(_):
    ...
    with tf.Session() as session:
        ...
        decode_logits_ph = tf.placeholder(
            tf.float32, [None, FLAGS.test_batch_size, alphabet.size() + 1])
        ...
        decoded, _ = decode_with_lm(
            decode_logits_ph, seq_lengths_ph, merge_repeated=False, beam_width=FLAGS.beam_width)
        ...
        mapping = {v.op.name: v for v in tf.global_variables(
        ...
        for batch in bar(split_data(test_data, FLAGS.test_batch_size)):
            ...
            full_step_len = np.full_like(batch_features_len, 16)
    ...
    wer, samples = calculate_report(
        ground_truths, predictions, distances, losses)
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string(
        'hdf5_test_set',
        '',
        'path to hdf5 file to cache test set features')
    tf.app.run(main)
